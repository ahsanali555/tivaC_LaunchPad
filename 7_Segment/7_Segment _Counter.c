...
int main(void)
{
	...
	PORTB_Init(); //Initializing PortB
  PORTF_Init(); //Initializing PortF

	GPIO_PORTB_DATA_R = digitPattern[i]; //The first Digit to display on 7-Segment
	
	while(1)
	{
    // Count up and DOWN Code
		 /*
		if ((GPIO_PORTF_DATA_RD & 0x10) == 0)
		{
			if (i>=9) i = -1;													// circle from 0 to 9
			GPIO_PORTB_DATA_R = digitPattern[++i];   	// display digits from 0 to 9 
			Delay(1000);																// debounce delay
		}
		else if ((GPIO_PORTF_DATA_RD & 0x01) == 0)
		{
			if (i==0) i = 10;													// circle from 0 to 9
			GPIO_PORTB_DATA_R = digitPattern[--i];   	// display digits from 0 to 9 
			Delay(1000);																// debounce delay
		}
		 */
